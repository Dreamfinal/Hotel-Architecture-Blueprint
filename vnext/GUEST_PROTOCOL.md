...
1. **Read Reception from the latest Hotel `control_ref`.** Fetch the authoritative remote control ref, read its Reception, and confirm the Hotel is `OPEN` and claims are enabled. Do not select work from a copied/stale Reception.
2. **Find a claimable Room.** Consider only dependency-ready Rooms listed by that Reception. Perform at most the allowed prefix-filtered remote claim lookup.
3. **Claim exactly one Room atomically.** Use the fixed Room claim ref defined by `CLAIM_PROTOCOL.md`, starting from that Room's exact `claim_base_sha` declared on the control ref.
...
5. **Enter the Room.** From the claimed branch/base, read only the claimed Room `START_HERE.md` and manifest.
...
7. **Do the work.** Write only the Room write allowlist. Preserve Hotel-wide forbidden write paths, locked contracts, and dependency outputs.
...
- the Hotel is not open or claims are disabled on the latest control ref;
...
- a required change falls outside the write allowlist or inside a Hotel-wide forbidden write path;
