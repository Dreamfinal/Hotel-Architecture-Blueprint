...
- **Guest is disposable.** A Guest does not need Team Repo, Staff memory, whole project history, or the Hotel blueprint.
- **Room is the execution boundary.** A Room supplies bounded task, compiled context, Room-local skills, tools/capabilities, authority, source/write scope, checks, acceptance, and return contract.
...
- `RETURN_PROTOCOL.md` — standard `ROOM_RETURN.json`, review, acceptance, rework, and integration.
- `RUNTIME_ADAPTERS.md` — ChatGPT and Codex implementations of the same Guest/control/claim/return contract.
...
- `schemas/RETURN.schema.json` — standard machine-readable Guest Room return.
- `schemas/HOTEL_HISTORY.schema.json` — minimal audit record retained after demolition with per-Room control/base/head/integration evidence.
- `templates/RECEPTION.md` — Guest-facing Reception tied to `control_ref`.
- `templates/ROOM_START_HERE.md` — compiled one-Room Guest entry tied to a control snapshot.
- `templates/ROOM_RETURN.json` — standard return template.
...
- `examples/minimal-project/` — dependency-aware example (`R001 → R002`) showing compiled control input vs claim-base source.
- `examples/records/` — Claim, Return, and demolished-Hotel history schema examples.
